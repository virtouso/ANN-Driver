...
public class VehicleControl : MonoBehaviour
{
    ...
    enum ControllerModes { Player, ANN }
    [SerializeField] private ControllerModes ControllerMode;
    [SerializeField] private bool _shouldTakeImages;
    [SerializeField] private Camera _playerCamera;
    [SerializeField] private Camera _aiCamera;
    private Photographer _photographer;
    private AnnWrapper _annWrapper;
    private Noedify.Net _net;
    ...
    void FixedUpdate()
    {
        ...
        if (activeControl)
        {
            ...
            if (controlMode == ControlMode.simple)
            {
                ...
                {
                    //todo moeen inputs
                    if (ControllerMode == ControllerModes.Player)
                    {
                        steer = Mathf.MoveTowards(steer, Input.GetAxis("Horizontal"), 0.2f);
                        accel = Input.GetAxis("Vertical");
                        brake = Input.GetButton("Jump");
                        shift = false;// Input.GetKey(KeyCode.LeftShift) | Input.GetKey(KeyCode.RightShift);
                        if (_shouldTakeImages)
                        {
                            TakeImage(_shotCounter++, steer, accel, boolToFloat(brake));


                        }
                    }

                    else if (ControllerMode == ControllerModes.ANN)
                    {
                        Texture2D txt = _photographer.TakeScreenShot();
                        float[,,] netInput = _annWrapper.generateFloatArray(txt.GetPixels());
                        _solver.Evaluate(_net, netInput);
                        float[] result = _solver.prediction;

                        steer = result[0];
                        accel = result[1];
                        brake = floatToBool(result[2]);
                    }
                }
    ...
    private void TakeImage(int index, float forward, float right, float backward)
    {


        _photographer.TakeScreenShotToFile(index, forward, right, backward);
    }
